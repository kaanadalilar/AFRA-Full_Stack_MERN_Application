...
export const register = (name, email, password, pic, username) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      ...
      { name, pic, email, password, username },
      config
    );
...
export const updateProfile = (user) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(`${process.env.REACT_APP_URL}/api/users/profile`, user, config);
...
export const deleteProfile = (user) => async (dispatch, getState) => {
  try {
    ...
    await axios.post(`${process.env.REACT_APP_URL}/api/users/deleteaccount`, user, config);
    dispatch({ type: USER_LOGOUT });
